...
class Two extends Component {
	...
	init() {
		...
		renderer.domElement.addEventListener('touchstart', this.startCube, false);
		renderer.domElement.addEventListener('touchmove', this.moveCube, false);
		renderer.domElement.addEventListener('touchend', this.stopCube, false);
	}
	...
	getIntersects(event) {
		// console.log('get Intersects event', event);
		if (event.touches) {
		...
		// console.log(raycaster, 'RRRRRRRRRRRRRRRRRRRRRRR');
		raycaster.setFromCamera(mouse, camera);
		// console.log(raycaster, 'RRRRRRRRRRRRRRRRRRRRRRR2');
		// console.log(scene.children, 'sceneCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCc');
		...
		if (intersects.length) {
			try {
				if (intersects[0].object.cubeType === 'coverCube') {
					...
					// normalize = intersects[0];
					normalize = new THREE.Vector3(0, 1, 0);
				} else {
					intersect = intersects[0];
					// normalize = intersects[1];
					normalize = new THREE.Vector3(0, 1, 0);
				}
			} catch (err) {
				//nothing
	...
	getBoxs(target, direction) {
		...
		for (var i = 0; i < meshesArray.length; i++) {
			ids.push(meshesArray[i].cubeIndex);
		}
		...
		switch (direction) {
			...
			case 3.4:
				for (var i = 0; i < meshesArray.length; i++) {
					var tempId = meshesArray[i].cubeIndex - minId;
					if (numI === parseInt(tempId / 9)) {
						boxs.push(meshesArray[i]);
					}
				}
				break;
			//绕y轴
	...
	moveCube(event) {
		...
		this.getIntersects(event);
		// console.log('mouseMove', intersect);
		if (intersect) {
			if (!isRotating && startPoint) {
				movePoint = intersect.point;
				console.log(movePoint, 'MPMPMMMMMMMMMMMMMMMMM');
				console.log(startPoint, 'SPSPSPSSSSSSSSSSSSSSSSSSSSS');
				if (!movePoint.equals(startPoint)) {
					console.log(startPoint, '【【【【【【【【【【【【【【【【【【【【【【【【');
					isRotating = true;
					var sub = movePoint.sub(startPoint);
					var direction = this.getDirection(sub);
					console.log(startPoint, '））））））））））））））））', direction);
					var elements = this.getBoxs(intersect, direction);
					var startTime = new Date().getTime();
					console.log('before request anim');
					var that = this;
					requestAnimationFrame(function(timestamp) {
						console.log('callback anim');
						that.rotateAnimation(elements, direction, timestamp, 0);
					});
				}
			}
		}
	...
	rotateAnimation(elements, direction, currentstamp, startstamp, laststamp) {
		...
		if (currentstamp - startstamp < totalTime) {
			requestAnimationFrame(function(timestamp) {
				_that.rotateAnimation(elements, direction, timestamp, startstamp, currentstamp);
			});
		}
	}
